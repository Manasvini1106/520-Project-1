...
def maze(board):  # defining the maze: a board with 70% unblocked cells and 30% blocked cells
    length = len(board)
...
class Astar:
    ...
    def __init__(self, size, window, start,end):  # The constructor: defining the size,window,distance,f,g,h,start and end points
        self.size = size
        self.window = window
        self.start = start
        self.end = end
    ...
    def repeated_forward_astar(self, array):
        ...
        f = {self.start: self.manhattan(self.start,
                                        self.end)}  # f value is the sum of g value and h value, meaning g and the
    ...
    '''Implementation of Repeated Backwards A*, the difference between Repeated Forward A* and Repeated Backward A* is that while 
     Forward A* goes from the start to the end node, Backward A* goes from the end to start nodes making it faster.'''
    ...
    '''Adaptive A* is the modified version of Forward A* search, adaptive A* updates the h-values using information from previous searches.
     It basically transforms consistent hvalues into more informed consistent h-values. This allows it  to find shortest paths in 
     state spaces where the action costs can increase over time since consistent h-values remain consistent after action cost increases. '''
